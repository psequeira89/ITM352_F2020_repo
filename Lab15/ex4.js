...
app.use(cookieParser("ITM352 rocks!"));
...
function process_login(req, res) {
    ...
    if (!bad_username && !bad_password) {
        ...
        res.cookie('last_login') = get_timestamp();
        ...
        //add username cookie
        res.cookie('username', this_user).send(str);
    }
...
//returns the greeting if there is a username
function get_greeting(username) {
    let str = "";
    if (typeof username != 'undefined') str += `Welcome, ${username}!\n`;
    return str;
}

//returns last login timestamp message
function get_last_login() {
    //if not first login, display last login
    if (last_login) return `Last login: ${last_login}`;
    else return `First Visit!`;
}
...
app.get("/use_session", function (request, response) {
    ...
    if (typeof request.session['id'] != 'undefined') sessionID = request.session['id'];
    else sessionID = 'not available';

    response.send(`Welcome, your session ID is ${sessionID}`);
...
app.get("/login", function (request, response) {
    ...
    str = `
        <body>
        <h1>${get_greeting(request.cookies['username'])}</h1>
